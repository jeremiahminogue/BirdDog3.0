<script lang="ts">
  ...
  async function load() {
    loading = true;
    try {
      items = await api.get("/opportunities/suppliers");
  ...
  async function addItem() {
    ...
    adding = true;
    try {
      await api.post("/opportunities/suppliers", {
        name: newName.trim(),
  ...
  async function saveField(item: Supplier, field: string, value: string | null) {
    try {
      await api.put(`/opportunities/suppliers/${item.id}`, {
  ...
  async function confirmBatchDelete() {
    deleting = true;
    try {
      const res = await api.post("/opportunities/suppliers/batch-delete", { ids: confirmDeleteIds });
  ...
  async function onDrop(e: DragEvent, idx: number) {
    ...
    try { await api.put("/opportunities/suppliers/reorder", { order }); }
...
<div class="rl-page">
  <div class="rl-header">
    <h1 class="rl-title">Suppliers</h1>
  ...
  {#if confirmDeleteIds.length > 0}
    ...
    <div class="rl-confirm-backdrop" on:click={cancelDelete}>
      <div class="rl-confirm-modal" on:click|stopPropagation>
        <h3 class="rl-confirm-title">Delete {confirmDeleteIds.length} supplier{confirmDeleteIds.length > 1 ? 's' : ''}?</h3>
        <p class="rl-confirm-text">This cannot be undone. Suppliers linked to opportunities cannot be deleted.</p>
  ...
  {:else}
    ...
    {#if items.length === 0}
      <div class="rl-empty-state">No suppliers yet. Add one below.</div>
    {/if}

    {#if canEdit}
      <div class="rl-add-bar">
        <input class="rl-add-input rl-add-name" type="text" bind:value={newName} placeholder="Supplier name" on:keydown={(e) => { if (e.key === 'Enter') addItem(); }} />
